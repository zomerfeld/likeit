...
# ********** Serial **********
usbport = '/dev/cu.usbmodem1441' # Need to change based on what it shows when connected. 
...
while True:
	...
	# frameClone = gray
	overlay = np.zeros((screenHeight,screenWidth,4), np.uint8) # EXP overlay maybe for rects on fullres? 
	imgresizer = screenWidth / resizeTo

	# loop over the face bounding boxes and draw them
	for (fX, fY, fW, fH) in faceRects:
		cv2.rectangle(grayorig, (fX*imgresizer, fY*imgresizer), (fX*imgresizer + fW*imgresizer, fY*imgresizer + fH*imgresizer), (0, 255, 0), 2)
		fWi = int(fW)
		fHi = int(fH)
		tempArea = fWi * fHi
	...
	if showvideo == 1:
		...
		grayorig = imutils.resize(grayorig, width = screenWidth, height=screenHeight)
		window_name = 'I LIKE IT WHEN YOU WATCH'
   		cv2.putText(grayorig, TextString, (110,310),cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255,0),2)		
   		cv2.namedWindow(window_name, cv2.WINDOW_GUI_EXPANDED)
		cv2.moveWindow(window_name, -1, -1)
   		cv2.setWindowProperty(window_name, cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN);
   		# cv2.setWindowProperty('I LIKE IT WHEN YOU WATCH', 0, 1);
		cv2.imshow(window_name, grayorig)
